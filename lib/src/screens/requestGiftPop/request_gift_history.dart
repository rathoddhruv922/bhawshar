...
import 'package:bhawsar_chemical/src/screens/requestGiftPop/widgets/request_list_widget.dart';
...
class RequestGiftPopHistoryScreen extends StatelessWidget {
  const RequestGiftPopHistoryScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    context.read<OrderBloc>()
      ...
      ..add(const GetOrdersEvent(currentPage: 1, recordPerPage: 20, orderType: 'MR'));
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        onPressed: () {
          navigationKey.currentState?.restorablePushNamed(RouteList.requestGiftPop);
        },
        icon: const Icon(
          Icons.add,
          color: white,
              ...
              listener: (context, state) async {
                if (state.status == OrderStatus.added) {
                  context.read<OrderBloc>().add(
                        const GetOrdersEvent(currentPage: 1, recordPerPage: 20, orderType: 'MR'),
                      );
              ...
              builder: (context, state) {
                if (state.status == OrderStatus.initial) {
                  ...
                  return state.res.items!.length == 0
                      ...
                      : RequestGiftListWidget(hasReachedMax: state.hasReachedMax, orderList: state.res);
                }
